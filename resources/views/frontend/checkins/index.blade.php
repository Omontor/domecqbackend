        ...
        <div class="col-md-12">
            @can('checkin_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.checkins.create') }}">
                            ...
                            <tbody>
                                @foreach($checkins as $key => $checkin)
                                    <tr data-entry-id="{{ $checkin->id }}">
                                        ...
                                        <td>
                                            @can('checkin_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.checkins.show', $checkin->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('checkin_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.checkins.edit', $checkin->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('checkin_delete')
                                                <form action="{{ route('frontend.checkins.destroy', $checkin->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('checkin_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.checkins.massDestroy') }}",
